...
Much like for the [hot routes tutorial](https://rekadata.site/blog/hot-routes-tutorial/), I will use the example dataset of the London Underground network, spatial data for which can be downloaded using the [Transport for London API](https://api-portal.tfl.gov.uk/docs), and crime from British Transport Police for February 2020 available via [data.police.uk](https://data.police.uk/). 
...
Now we can snap each crime point to the closest tube station using the `st_nearest_feature()` function. This will return, for each point, the ID of the nearest segment. We can do this once for violent crime and one for other crimes
...
Once we have our data, we want specifically the excel file "Total entries and exits by borough-time of day 2017.xls". The entry and exit data sets are two separate sheets in this excel file, so we will have to read them in one sheet at a time, using `read_xls()` function from the `readxl` package. From having seen the data before, I also know that 1) the variable names are in line 4 so we can skip up to there (the `skip=` argument in the `read_xls()` function), and I know also that the names are messy, so we clean them up using the `clean_names()` function in the `janitor` package. Finally for some reason the 'station' variable is in two columns, so we have to name these as station number and station name using `rename()` function in the `dplyr` library (loaded earlier). It seems like a lot, but we're really just cleaning the data to suit our purposes. 
...
Ha-ha this dataset has `r nrow(bakerloo_stops_try1 %>% filter(is.na(total_pax)))` NA values, that is for `r nrow(bakerloo_stops_try1 %>% filter(is.na(total_pax)))` stations we did not find a corresponding match! Well we can have a look at which stations don't match: 

```{r viewthisshiz, eval=FALSE}

View(bakerloo_stops_try1 %>% filter(is.na(total_pax)))
...
```{r viewthisshiz2, eval=FALSE}

View(bakerloo_stops_try2 %>% filter(is.na(total_pax)))
